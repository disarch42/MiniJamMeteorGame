...
public class Meteor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        trail.transform.rotation = Quaternion.Slerp(trail.transform.rotation, Quaternion.Euler( new Vector3(0, 0, rad*Mathf.Rad2Deg+180)), Time.deltaTime*trailTurnSpeed);
        trail.transform.position = Vector3.Lerp(trail.transform.position, 
            transform.position - new Vector3(Mathf.Cos(rad) * radius * 1.1f, Mathf.Sin(rad) * radius * 1.1f, 0), trailMoveSpeed*Time.deltaTime);
    ...
    public void DamageMeteor(float dmg)
    {
        ...
        if (health < 0)
        {
            ...
            if (isBomb)
            {
                ...
                s.Initialize(transform.position, .5f, explosionRadius);
                for (int i = 0; i < GameManager.GetInstance().meteors.Count; i++)
                {
                    if (GameManager.GetInstance().meteors[i] == this) { continue; }
                    if (Vector2.Distance(GameManager.GetInstance().meteors[i].transform.position, transform.position)-GameManager.GetInstance().meteors[i].radius < explosionRadius)
                    {
