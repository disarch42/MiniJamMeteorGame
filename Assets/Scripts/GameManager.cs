...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (InputSystemClass.GetCurrent() != null)
        {
            InputSystemClass.GetCurrent().actions.Player.BlackHole.performed += OnBlackHolePerformed;
            InputSystemClass.GetCurrent().actions.Player.BlackHole.canceled += OnBlackHoleCanceled;
        }
    }
            ...
            for (int i = 0; i < meteors.Count; i++)
            {
                for (int j = i + 1; j < meteors.Count; j++)
                {
                    ...
                    if (dist < (meteors[i].radius + meteors[j].radius))
                    {
                        ...
                        SoundManager.current.PlayOneShot(SoundManager.current.crashClip);
        ...
        void BlackHoleUpdate()
        {
            ...
            if (charging)
            {
                ...
                if (!_holdingM1)
                {
                    ...
                    SoundManager.current.PlayOneShot(SoundManager.current.explosionClip);
